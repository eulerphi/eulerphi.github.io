...
{
	if (region.aw.Z === region.aF.Z)
	{
		return 'on line ' + region.aw.Z;
	}
	return 'on lines ' + region.aw.Z + ' through ' + region.aF.Z;
}
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { bo: 'hidden', bi: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { bo: 'mozHidden', bi: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { bo: 'msHidden', bi: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { bo: 'webkitHidden', bi: 'webkitvisibilitychange' }
		: { bo: 'hidden', bi: 'visibilitychange' };
}
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {aJ: fragment, aM: host, aV: path, aY: port_, a0: protocol, a1: query};
	});
...
var $author$project$Input$RawInput = F5(
	function (devicePixelRatio, explanation, fen, moves, prompt) {
		return {V: devicePixelRatio, W: explanation, X: fen, _: moves, ab: prompt};
	});
...
var $author$project$Input$decode = function (value) {
	...
	if (!_v0.$) {
		...
		return {V: 1.0, W: '', X: '', _: _List_Nil, ab: '<Input-Malformed>'};
	}
};
...
var $author$project$ViewContext$makeGetViewpointCmd = function (vc) {
	return A2(
		$elm$core$Task$perform,
		A2($elm$core$Basics$composeL, vc.bm, $author$project$ViewContext$ViewportChanged),
		$elm$browser$Browser$Dom$getViewport);
};
var $author$project$ViewContext$initCmd = function (vc) {
	return $author$project$ViewContext$makeGetViewpointCmd(vc);
};
...
var $romstad$elm_chess$Internal$Board$ReadFenState = F3(
	function (board, fileIndex, rankIndex) {
		return {ai: board, D: fileIndex, al: rankIndex};
	});
...
var $romstad$elm_chess$Internal$Board$readFenPiece = F2(
	function (piece, state) {
		return A3(
			...
			piece,
			$romstad$elm_chess$Internal$Square$expand(state.D + (8 * state.al)),
			state.ai);
	});
	...
	function (_char, state) {
		...
		if (!_v0.$) {
			...
			return _Utils_update(
				state,
				{
					ai: A2($romstad$elm_chess$Internal$Board$readFenPiece, piece, state),
					D: state.D + 1
				});
		} else {
...
var $romstad$elm_chess$Internal$Position$pieceOn = F2(
	function (square, pos) {
		return A2(
			$romstad$elm_chess$Internal$Board$pieceOn,
			square,
			$romstad$elm_chess$Internal$Position$unwrap(pos).ai);
	});
...
var $romstad$elm_chess$Internal$Position$doMove = F2(
	function (move, position) {
		...
		var pos = position;
		return {
			v: _Utils_eq(piece, $romstad$elm_chess$Internal$Piece$blackKing) ? $elm$core$Maybe$Just(to) : pos.v,
			ai: A2($romstad$elm_chess$Internal$Board$doMove, move, pos.ai),
			w: A2($romstad$elm_chess$Internal$CastleRights$doMove, move, pos.w),
...
var $romstad$elm_chess$Internal$Position$kingSquare = F2(
	function (color, position) {
		return _Utils_eq(color, $romstad$elm_chess$Internal$PieceColor$white) ? $romstad$elm_chess$Internal$Position$unwrap(position).C : $romstad$elm_chess$Internal$Position$unwrap(position).v;
	});
...
var $romstad$elm_chess$Internal$Position$sideAttacksSquare = F3(
	function (side, square, position) {
		return A3(
			...
			side,
			square,
			$romstad$elm_chess$Internal$Position$unwrap(position).ai);
	});
...
var $romstad$elm_chess$Internal$Position$canCastleKingside = F2(
	function (side, position) {
		return A2(
			...
			side,
			$romstad$elm_chess$Internal$Position$unwrap(position).w);
	});
...
var $author$project$Step$makeSteps = F4(
	function (number, position, prevMove, sans) {
		if (!sans.b) {
			return _List_fromArray(
				[
					{aT: $elm$core$Maybe$Nothing, bt: number, bx: position, by: prevMove}
				]);
		} else {
			var x = sans.a;
			var xs = sans.b;
			...
			var step = {aT: move, bt: number, bx: position, by: prevMove};
			var tail = A2(
				$elm$core$Maybe$map,
				function (p) {
				...
				A2(
					$elm$core$Maybe$map,
					function (m) {
						return A2($romstad$elm_chess$Position$doMove, m.aT, position);
					},
					move));
			return A2(
				$elm$core$Maybe$withDefault,
	...
	return $elm$core$Array$fromList(
		A2(
			...
			A2(
				$elm$core$Maybe$map,
				function (p) {
					return A4($author$project$Step$makeSteps, 1, p, $elm$core$Maybe$Nothing, input._);
				},
				$romstad$elm_chess$Position$fromFen(input.X))));
};
var $author$project$ViewContext$ViewContext = F3(
	function (devicePixelRatio, envelope, size) {
		return {V: devicePixelRatio, bm: envelope, am: size};
	});
var $author$project$Size$Size = F2(
	function (width, height) {
		return {ao: height, bG: width};
	});
var $author$project$Size$none = A2($author$project$Size$Size, 0, 0);
var $author$project$ViewContext$init = function (input) {
	return A3($author$project$ViewContext$ViewContext, input.V, input.bm, $author$project$Size$none);
};
var $author$project$Board$none = {U: 0, an: 0, as: 0, at: 0, ac: 0, ay: ''};
var $romstad$elm_chess$PieceColor$white = $romstad$elm_chess$Internal$PieceColor$white;
var $author$project$Main$initFromInput = function (input) {
	var steps = $author$project$Step$fromInput(input);
	var current = {
		P: 0,
		ad: A2($elm$core$Array$get, 0, steps)
	};
	var playerColor = A2(
		$elm$core$Maybe$withDefault,
		...
		A2(
			...
			A2(
				...
				current.ad)));
	return {
		ai: $author$project$Board$none,
		x: current,
		aR: $elm$core$Dict$empty,
		aX: playerColor,
		ab: input.ab,
		R: steps,
		K: $author$project$ViewContext$init(
			{V: input.V, bm: $author$project$Main$ViewCtxMsg})
	};
};
var $author$project$Main$init = function (flags) {
	var m = $author$project$Main$initFromInput(
		$author$project$Input$decode(flags));
	return _Utils_Tuple2(
		m,
		$author$project$ViewContext$initCmd(m.K));
};
...
var $author$project$ViewContext$subscriptions = function (vc) {
	return $elm$browser$Browser$Events$onResize(
		F2(
			function (_v0, _v1) {
				return vc.bm($author$project$ViewContext$WindowResized);
			}));
};
var $author$project$Main$subscriptions = function (m) {
	return $elm$core$Platform$Sub$batch(
		_List_fromArray(
			[
				$author$project$ViewContext$subscriptions(m.K)
			]));
};
var $author$project$Board$NoMark = 0;
...
var $author$project$Board$fromViewContext = F2(
	function (board, vc) {
		...
		var headerHeight = 0.1 * vc.am.ao;
		var footerHeight = 0.2 * vc.am.ao;
		var contentWidth = vc.am.bG - minMarginWidths;
		var boardSize = A2($elm$core$Basics$min, (vc.am.ao - headerHeight) - footerHeight, 0.6 * contentWidth);
		var panelHeight = boardSize / 2;
		var panelWidth = A2($elm$core$Basics$min, 0.4 * contentWidth, boardSize / 2);
		var squareSize = boardSize / 8;
		return _Utils_update(
			board,
			{U: boardSize, an: headerHeight, as: panelHeight, at: panelWidth, ac: squareSize});
	});
...
var $author$project$Board$Green = 1;
var $author$project$Board$Red = 2;
var $author$project$Board$nextMark = function (current) {
	switch (current) {
		case 0:
			return 1;
		case 1:
...
var $author$project$ViewContext$updateOnSizeChanged = F2(
	function (_v0, vc) {
		...
		return _Utils_update(
			vc,
			{
				am: A2($author$project$Size$Size, viewport.bG, viewport.ao)
			});
	});
	...
	function (msg, vc) {
		switch (msg.$) {
			...
			default:
				return _Utils_Tuple2(
					vc,
					$author$project$ViewContext$makeGetViewpointCmd(vc));
		}
	});
	...
	function (msg, m) {
		switch (msg.$) {
			...
			case 2:
				...
				var val = $author$project$Board$nextMark(
					A2(
						$elm$core$Maybe$withDefault,
						0,
						A2($elm$core$Dict$get, key, m.aR)));
				var marks_ = (!(!val)) ? A3($elm$core$Dict$insert, key, val, m.aR) : A2($elm$core$Dict$remove, key, m.aR);
				var m_ = _Utils_update(
					m,
					{
						aR: marks_,
						ab: $elm$core$String$fromInt(key)
					});
			...
			case 3:
				var idx_ = A2(
					$elm$core$Basics$min,
					m.x.P + 1,
					$elm$core$Array$length(m.R) - 1);
				var step_ = A2($elm$core$Array$get, idx_, m.R);
				var m_ = _Utils_update(
					m,
					{
						x: {P: idx_, ad: step_},
				...
				return _Utils_Tuple2(m_, $elm$core$Platform$Cmd$none);
			case 4:
				var idx_ = A2($elm$core$Basics$max, 0, m.x.P - 1);
				var step_ = A2($elm$core$Array$get, idx_, m.R);
				var m_ = _Utils_update(
					m,
					{
			...
			default:
				var subMsg = msg.a;
				var _v1 = A2($author$project$ViewContext$update, subMsg, m.K);
				var vc_ = _v1.a;
				var cmd_ = _v1.b;
				return _Utils_Tuple2(
					_Utils_update(
						m,
						{
							ai: A2($author$project$Board$fromViewContext, m.ai, vc_),
							K: vc_
						}),
					cmd_);
		}
...
var $author$project$Main$ClearMarks = {$: 1};
var $author$project$Main$MarkSquare = function (a) {
	return {$: 2, a: a};
};
var $author$project$Main$NextMove = {$: 3};
var $author$project$Main$PrevMove = {$: 4};
...
var $author$project$StringEx$pixelLength = function (length) {
...
var $author$project$Board$boardSvgUri = function (name) {
	switch (name) {
		case 'blue':
...
var $author$project$Board$Pos = F2(
	function (rank, file) {
		return {aj: file, ak: rank};
	});
...
var $author$project$Board$fileCoords = function () {
	var start = $elm$core$Char$toCode('a');
	return A2(
		$elm$core$List$map,
		function (f) {
			return _Utils_Tuple2(
				A2($author$project$Board$Pos, 0, f),
				$elm$core$String$fromChar(
					$elm$core$Char$fromCode((start + f) - 1)));
		},
		A2($elm$core$List$range, 1, 8));
}();
var $author$project$Board$rankCoords = A2(
	$elm$core$List$map,
	function (r) {
		return _Utils_Tuple2(
			A2($author$project$Board$Pos, r, 0),
			$elm$core$String$fromInt(r));
	},
	A2($elm$core$List$range, 1, 8));
var $author$project$Board$translate = F2(
	function (board, pos) {
		var y = board.ac * (8 - pos.ak);
		var x = board.ac * (pos.aj - 1);
		return A2(
			$elm$core$String$join,
			'',
			_List_fromArray(
...
var $author$project$Board$viewCoord = F2(
	function (board, _v0) {
		var pos = _v0.a;
		var text = _v0.b;
		return A2(
			...
			_List_fromArray(
				[
					...
					'transform',
					A2($author$project$Board$translate, board, pos)),
...
var $author$project$Board$viewCoords = function (board) {
	return A2(
		$elm$core$List$map,
		$author$project$Board$viewCoord(board),
		_Utils_ap($author$project$Board$rankCoords, $author$project$Board$fileCoords));
};
...
var $author$project$Board$pieceSvgUri = function (code) {
	switch (code) {
		case 'b':
...
var $author$project$Board$toPos = function (square) {
	return {
		aj: 1 + $romstad$elm_chess$SquareFile$toIndex(
			$romstad$elm_chess$Square$file(square)),
		ak: 1 + $romstad$elm_chess$SquareRank$toIndex(
			$romstad$elm_chess$Square$rank(square))
	};
};
var $author$project$Board$viewPiece = F3(
	function (board, square, piece) {
		if (piece.$ === 1) {
			return _List_Nil;
		} else {
			var p = piece.a;
			var div = A2(
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$style,
						'background-image',
						$author$project$Board$pieceSvgUri(
						...
						'transform',
						A2(
							$author$project$Board$translate,
							board,
							$author$project$Board$toPos(square))),
						A2($elm$html$Html$Attributes$style, 'position', 'absolute'),
						A2($elm$html$Html$Attributes$style, 'top', '0'),
						A2($elm$html$Html$Attributes$style, 'left', '0')
					]),
...
var $author$project$Board$viewPieces = F2(
	function (board, position) {
		return A3(
			...
			A2(
				...
				function (s) {
					return A3(
						$author$project$Board$viewPiece,
						board,
						s,
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					$elm$html$Html$Attributes$style,
					'background-image',
					$author$project$Board$playerIndicatorSvgUri(whiteToPlay)),
					A2($elm$html$Html$Attributes$style, 'background-repeat', 'no-repeat'),
...
var $author$project$Board$viewMark = F2(
	function (input, mark) {
		var color = function () {
			switch (mark) {
				case 0:
...
var $author$project$Board$viewSquare = F3(
	function (input, prevMove, square) {
		var moveCss = function () {
			if (prevMove.$ === 1) {
				...
				var pm = prevMove.a;
				return (_Utils_eq(
					$romstad$elm_chess$Move$from(pm.aT),
					square) || _Utils_eq(
					$romstad$elm_chess$Move$to(pm.aT),
					square)) ? _List_fromArray(
					[
...
var $author$project$Board$viewSquares = F2(
	function (input, prevMove) {
		return A2(
			$elm$core$List$map,
			A2($author$project$Board$viewSquare, input, prevMove),
			$romstad$elm_chess$Square$all);
	});
var $author$project$Board$view = F2(
	function (input, position) {
		var whiteToPlay = _Utils_eq(input.aX, $romstad$elm_chess$PieceColor$white);
		var board = input.ai;
		if (position.$ === 1) {
			...
			return A2(
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$style,
						'width',
						$author$project$StringEx$pixelLength(board.U)),
						A2(
						$elm$html$Html$Attributes$style,
						'height',
						$author$project$StringEx$pixelLength(board.U)),
						...
						$elm$html$Html$Attributes$style,
						'background-image',
						$author$project$Board$boardSvgUri(board.ay)),
						A2($elm$html$Html$Attributes$style, 'background-repeat', 'no-repeat')
					]),
				_List_fromArray(
					[
						...
						$elm$html$Html$div,
						_List_Nil,
						$author$project$Board$viewCoords(board)),
						A2(
						$elm$html$Html$div,
						_List_Nil,
						_List_fromArray(
							[
								A2($author$project$Board$viewPlayerIndicator, board, whiteToPlay)
							])),
						A2(
						$elm$html$Html$div,
						_List_Nil,
						A2($author$project$Board$viewSquares, input, s.by)),
						A2(
						$elm$html$Html$div,
						_List_Nil,
						A2($author$project$Board$viewPieces, input.ai, s.bx))
					]));
		}
	});
var $author$project$MaybeEx$hasValue = function (m) {
	if (!m.$) {
		return true;
	} else {
		return false;
...
var $author$project$Main$viewLogStep = F3(
	function (stepIdx, idx, step) {
		var text = (_Utils_cmp(idx, stepIdx) < 0) ? A2(
			$elm$core$Maybe$withDefault,
			'',
			A2(
				$elm$core$Maybe$map,
				...
				step.aT)) : '???';
		var numberSuffix = _Utils_eq(
			$romstad$elm_chess$Position$sideToMove(step.bx),
			$romstad$elm_chess$PieceColor$white) ? '.  ' : '...';
		var _v0 = _Utils_eq(stepIdx, idx) ? _Utils_Tuple2(
			_List_fromArray(
				[
		...
		var highlightCss = _v0.a;
		var highlightContent = _v0.b;
		return A2(
			$elm$html$Html$tr,
			highlightCss,
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text(highlightContent)
						])),
					A2(
					$elm$html$Html$td,
					_List_fromArray(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text(
									$elm$core$String$fromInt(step.bt))
								])),
							A2(
							$elm$html$Html$span,
							_List_Nil,
							_List_fromArray(
								[
									$elm$html$Html$text(numberSuffix)
								]))
						])),
					A2(
					$elm$html$Html$td,
...
var $author$project$Main$viewLog = F2(
	function (idx, steps) {
		return A2(
			...
			$author$project$Main$viewLogStep(idx),
			A2(
				$elm$core$List$filter,
				function (s) {
					return $author$project$MaybeEx$hasValue(s.aT);
				},
				$elm$core$Array$toList(steps)));
	});
			...
			[
				...
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$style,
						'height',
						$author$project$StringEx$pixelLength(m.ai.an)),
						A2(
						$elm$html$Html$Attributes$style,
						'width',
						$elm$core$String$fromFloat(0.9 * m.K.am.bG) + 'px'),
						A2($elm$html$Html$Attributes$style, 'display', 'flex'),
						A2($elm$html$Html$Attributes$style, 'align-items', 'center'),
						A2($elm$html$Html$Attributes$style, 'justify-content', 'center'),
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								A2($elm$html$Html$Attributes$style, 'padding', '20px 20px'),
								$elm$html$Html$Events$onClick($author$project$Main$ClearMarks)
							]),
						_List_fromArray(
							[
								$elm$html$Html$text('clear marks')
							])),
						A2(
						$elm$html$Html$button,
						_List_fromArray(
							[
								...
								$elm$html$Html$Events$onClick($author$project$Main$PrevMove)
							]),
						_List_fromArray(
							[
								$elm$html$Html$text('< prev move')
							])),
						A2(
						$elm$html$Html$button,
						_List_fromArray(
							[
								A2($elm$html$Html$Attributes$style, 'padding', '20px 20px'),
								$elm$html$Html$Events$onClick($author$project$Main$NextMove)
							]),
						_List_fromArray(
							[
							...
							[
								...
								_List_fromArray(
									[
										...
										$elm$html$Html$Attributes$style,
										'height',
										$author$project$StringEx$pixelLength(m.ai.as)),
										A2(
										$elm$html$Html$Attributes$style,
										'width',
										$author$project$StringEx$pixelLength(m.ai.at)),
								...
								_List_fromArray(
									[
										...
										_List_fromArray(
											[
												$elm$html$Html$text(m.ab)
											])),
										A2(
										$elm$html$Html$table,
										_List_Nil,
										A2($author$project$Main$viewLog, m.x.P, m.R))
									])),
								A2(
								$elm$html$Html$td,
								_List_fromArray(
									[
										...
										$elm$html$Html$Attributes$style,
										'height',
										$author$project$StringEx$pixelLength(m.ai.U)),
										A2(
										$elm$html$Html$Attributes$style,
										'width',
										$author$project$StringEx$pixelLength(m.ai.U)),
										...
										$elm$html$Html$Attributes$style,
										'padding-left',
										$author$project$StringEx$pixelLength(m.ai.ac))
									]),
								_List_fromArray(
									[
										A2(
										...
										_List_fromArray(
											[
												A2(
												$author$project$Board$view,
												{ai: m.ai, aC: $author$project$Main$MarkSquare, aR: m.aR, aX: m.aX},
												m.x.ad)
											]))
									]))
							]))
...
var $author$project$Main$main = $elm$browser$Browser$element(
	{bq: $author$project$Main$init, bB: $author$project$Main$subscriptions, bD: $author$project$Main$update, bE: $author$project$Main$view});
